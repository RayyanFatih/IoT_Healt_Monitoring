    ...
    <form class="auth-form" id="regForm" onsubmit="handleRegister(event)" novalidate>
      ...
      <div class="form-group">
        ...
        <div class="input-wrap">
          ...
          <button type="button" class="pw-toggle" id="toggleRegPw" onclick="togglePassword('reg-password','toggleRegPw')">
      ...
      <div class="form-group">
        ...
        <div class="input-wrap">
          ...
          <button type="button" class="pw-toggle" id="toggleConfirm" onclick="togglePassword('reg-confirm','toggleConfirm')">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>
          </button>
        </div>
...
function checkStrength(val) {
  ...
  const colorMap = { 1:'#ef4444', 2:'#f59e0b', 3:'#6366f1', 4:'#10b981' };
  ...
  bars.forEach((b, i) => { b.style.background = i < score ? colorMap[score] : '#e2e8f0'; });
...
async function handleRegister(e) {
  ...
  if (!name.value.trim()) {
    document.getElementById('name-error').classList.add('show'); name.classList.add('error'); valid = false;
  }
  if (!email.value || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email.value)) {
    const el = document.getElementById('reg-email-error');
    el.querySelector('span').textContent = 'Masukkan email yang valid';
    el.classList.add('show'); email.classList.add('error'); valid = false;
  }
  if (!pw.value || pw.value.length < 8) {
    document.getElementById('reg-pw-error').classList.add('show'); pw.classList.add('error'); valid = false;
  }
  if (pw.value !== confirm.value) {
    document.getElementById('confirm-error').classList.add('show'); confirm.classList.add('error'); valid = false;
  }
  if (!valid) return;
  ...
  try {
    const res = await fetch('/register', {
      ...
      body: JSON.stringify({ name: name.value.trim(), email: email.value, password: pw.value, password_confirmation: confirm.value }),
    });
    ...
    if (data.success) {
      ...
      if (data.errors?.email) {
        const el = document.getElementById('reg-email-error');
        el.querySelector('span').textContent = data.errors.email[0]; el.classList.add('show');
      }
      if (data.errors?.name) document.getElementById('name-error').classList.add('show');
      if (data.errors?.password) document.getElementById('reg-pw-error').classList.add('show');
      if (!data.errors) {
        const el = document.getElementById('reg-email-error');
        el.querySelector('span').textContent = data.message || 'Terjadi kesalahan.'; el.classList.add('show');
      }
      btn.disabled = false; btn.style.opacity = '1';
    }
  } catch {
    const el = document.getElementById('reg-email-error');
    el.querySelector('span').textContent = 'Terjadi kesalahan server. Coba lagi.'; el.classList.add('show');
    btn.disabled = false; btn.style.opacity = '1';
  }
}
